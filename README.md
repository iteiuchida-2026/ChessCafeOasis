...
| 8日目|6/12(金) | 計画内容に基づいて各スクリプトの詳細部分実装 | 実装作業| 1| ☑|（授業なし/就職支援） 体調不良で作業の大幅な遅れあり。|
| 9日目|6/13(土) | スクリプト実装作業| 実装作業| 1| ☑| 体調不良で作業の大幅な遅れあり。|
|10日目|6/14(日) | スクリプト実装作業| 実装作業| 1| ☑| 体調不良で作業の大幅な遅れあり。 |
|11日目|6/15(月) | チェスルールに沿った動作実装の調整 | ■調整テスト| 1| ☑| 体調不良で作業の大幅な遅れあり。 |
|12日目|6/16(火) | チェスルールに沿った動作実装のプレイテストと調整| ■調整テスト| 1|||
...
|6/15(月)| Blenderによるチェスセットの作成、Unityへのデータ移行まで完了。| チェスの駒は問題なく移動できた。チェス盤周りは再度作成し個別に移行。体調不良で学校は欠席する。| 6|<img width="1032" height="671" alt="タイトルなし3" src="https://github.com/user-attachments/assets/3797ecff-493a-496f-b96d-e3a524fe559b" />|
||||||
||||||
||||||
...
|------|-----------|-----------|------|------|
| 6/14(日) | エラーではないが、BlenderのマテリアルはUnityへインポート時に引き継がれない。 | チェス盤のオブジェクト。 | Unity側でマテリアルを付ける予定。 | AIへの相談の仕方はできるだけ全工程を前提として入れるべき。|
||||||
---
